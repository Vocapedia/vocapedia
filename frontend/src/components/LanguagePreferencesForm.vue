<template>
    <div class="space-y-4">
        <!-- Known Languages -->
        <div>
            <label class="block text-sm font-medium mb-2">{{ $t('account.popup.language_preferences.known_languages')
                }}</label>
            <div class="space-y-2">
                <div v-for="(lang, index) in knownLanguages" :key="'known-' + index"
                    ...
                    <select v-model="knownLanguages[index]"
                        ...
                        <option value="">{{ $t('account.popup.language_preferences.select.language') }}</option>
                        <option v-for="language in languages" :key="language.code" :value="language.code">
                            {{ language.name }}
                        </option>
                    </select>
                    <button @click="removeKnownLanguage(index)" class="p-2 text-red-500 hover:text-red-700">
                ...
                <button @click="addKnownLanguage"
                    ...
                    + {{ $t('account.popup.language_preferences.add.known_language') }}
                </button>
            </div>
        </div>
        ...
        <!-- Target Languages -->
        <div>
            <label class="block text-sm font-medium mb-2">{{ $t('account.popup.language_preferences.target_languages')
                }}</label>
            <div class="space-y-2">
                <div v-for="(lang, index) in targetLanguages" :key="'target-' + index"
                    class="flex items-center space-x-2">
                    ...
                    <button @click="removeTargetLanguage(index)" class="p-2 text-red-500 hover:text-red-700">
                        <mdicon name="minus-circle" />
                    </button>
                </div>
                <button @click="addTargetLanguage"
                    class="w-full px-3 py-2 border-2 border-dashed border-zinc-300 dark:border-zinc-600 rounded-md text-zinc-500 dark:text-zinc-400 hover:border-zinc-400 dark:hover:border-zinc-500">
                    + {{ $t('account.popup.language_preferences.add.target_language') }}
                </button>
            </div>
        </div>
    </div>
...
const saveLanguagePreferences = async () => {
    ...
    if (filteredKnown.length === 0 || filteredTarget.length === 0) {
        toast.error(t('account.popup.language_preferences.validation.select_at_least_one'));
        return false;
    }
    ...
    if (knownDuplicates.length > 0) {
        toast.error(t('account.popup.language_preferences.validation.no_duplicates_known'));
        return false;
    }
    ...
    if (targetDuplicates.length > 0) {
        toast.error(t('account.popup.language_preferences.validation.no_duplicates_target'));
        return false;
    }
    ...
    if (hasOverlap) {
        toast.error(t('account.popup.language_preferences.validation.no_overlap'));
        return false;
    }
    ...
    if (response.message) {
        ...
        toast.error(t('shared.error.something_went_wrong'));
    }
