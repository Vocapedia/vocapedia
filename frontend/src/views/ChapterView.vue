...
import ChapterTutorial from "@/components/chapter/ChapterTutorial.vue"
import ChapterWordList from "@/components/chapter/ChapterWordList.vue"
...
onMounted(async () => {
    ...
    if (route.query.variant == 'tutorial') {
        currentComponent.value = ChapterTutorial
    } else {
        currentComponent.value = ChapterWordList
    }
})
...
const currentComponent = shallowRef(ChapterWordList)
watch(route, (newV) => {
